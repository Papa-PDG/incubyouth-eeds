...
import { LayoutDashboard, Users, MessageSquare, Bot, FileText, ArrowLeft, BookOpen, MessagesSquare } from "lucide-react";
      ...
      <aside className="fixed left-0 top-16 z-30 hidden h-[calc(100vh-4rem)] w-[240px] flex-col bg-[#1F1535] text-white md:flex">
        ...
        <nav className="flex-1 space-y-1 px-3">
          {items.map((it) => {
            ...
            return (
              <Link key={it.to} to={it.to as "/admin"} className={className}>
                <Icon className="h-4 w-4" />
                {it.label}
              </Link>
            );
